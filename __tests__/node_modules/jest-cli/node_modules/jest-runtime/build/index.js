...
class Runtime {
  ...
  options) 
  {
    ...
    if (!this._moduleRegistry[modulePath]) {
      ...
      if (path.extname(modulePath) === '.json') {
        ...
        fs.readFileSync(modulePath, 'utf8'));} else 

      if (path.extname(modulePath) === '.node') {
        // $FlowFixMe
        localModule.exports = require(modulePath);} else 
      {
  ...
  requireMock(from, moduleName) {
    ...
    if (manualMockResource) {
      modulePath = this._resolveModule(from, manualMockResource);} else 
    {
      modulePath = this._resolveModule(from, moduleName);
    ...
    if (manualMockResource) {
      ...
      this._mockRegistry[moduleID] = localModule.exports;} else 
    {
  ...
  requireModuleOrMock(from, moduleName) {
    if (this._shouldMock(from, moduleName)) {
      return this.requireMock(from, moduleName);} else 
    {
